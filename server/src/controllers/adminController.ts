...
// ---------------------------------------------------------------------------
// Sweep Coin codes — AMOE (mail-in, no-purchase) and promo grants.
// The admin issues a code for each mail-in entry received, or a shared promo
// code. Redeeming a code is a FREE way to get Sweep Coins (the redeemable
// currency), which is what keeps the model a sweepstakes and not gambling.
// ---------------------------------------------------------------------------

const issueSweepCodeSchema = z.object({
  amountCents: z.number().int().min(1).max(100000),
  source: z.enum(['amoe', 'promo']).default('amoe'),
  maxRedemptions: z.number().int().min(1).max(100000).default(1),
  perUserOnce: z.boolean().default(true),
  note: z.string().max(200).optional(),
  expiresAt: z.string().datetime().optional(),
  code: z.string().min(3).max(32).optional(), // optional custom code, else auto-generated
});

function generateSweepCode(): string {
  // Human-friendly, ambiguity-free alphabet (no 0/O/1/I).
...
export async function issueSweepCode(req: AuthRequest, res: Response): Promise<void> {
  try {
    if (!(await requireAdmin(req, res))) return;

    const parsed = issueSweepCodeSchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({ error: parsed.error.errors[0].message });
      return;
    }
    const { amountCents, source, maxRedemptions, perUserOnce, note, expiresAt } = parsed.data;
    const code = (parsed.data.code || generateSweepCode()).trim().toUpperCase();

    const existing = await query(`SELECT 1 FROM sweep_codes WHERE code = $1`, [code]);
    if (existing.rows.length > 0) {
      res.status(400).json({ error: 'That code already exists. Try again.' });
      return;
    }

    const result = await query(
      `INSERT INTO sweep_codes (code, amount, source, max_redemptions, per_user_once, note, expires_at, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [code, amountCents, source, maxRedemptions, perUserOnce, note || null, expiresAt || null, req.user!.id],
    );

    res.status(201).json({ code: formatSweepCode(result.rows[0]) });
  } catch (err) {
    console.error('issueSweepCode error:', err);
    res.status(500).json({ error: 'Internal server error.' });
  }
}

export async function listSweepCodes(req: AuthRequest, res: Response): Promise<void> {
  try {
    if (!(await requireAdmin(req, res))) return;

    const result = await query(
      `SELECT * FROM sweep_codes ORDER BY created_at DESC LIMIT 200`,
    );
    res.json({ codes: result.rows.map(formatSweepCode) });
  } catch (err) {
    console.error('listSweepCodes error:', err);
    res.status(500).json({ error: 'Internal server error.' });
  }
}

function formatSweepCode(row: any) {
  return {
    id: row.id,
    code: row.code,
    amountCents: parseInt(row.amount, 10),
    source: row.source,
    maxRedemptions: parseInt(row.max_redemptions, 10),
    redemptionCount: parseInt(row.redemption_count, 10),
    perUserOnce: row.per_user_once,
    note: row.note,
